...
class AddIpdSurgeryReportPermission extends Migration
{
    private const PERMISSION = 'reports.ipd_surgery.view';
    ...
    public function up()
    {
        ...
        foreach (['superadmin', 'admin', 'developer'] as $alias) {
            ...
            $exists = $this->db->table('auth_role_permissions')->where('role_id', (int) $role['id'])->where('permission', self::PERMISSION)->countAllResults();
            if ($exists === 0) {
                $this->db->table('auth_role_permissions')->insert(['role_id' => (int) $role['id'], 'permission' => self::PERMISSION, 'created_at' => $now]);
            }
        }
