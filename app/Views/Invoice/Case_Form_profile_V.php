<section class="content-header">
    ...
    <ol class="breadcrumb">
        <li><a href="javascript:load_form('<?= base_url('billing/patient/person_record') ?>/<?=$org_case_master[0]->p_id ?>');"><i
...
$(document).ready(function() {
    $('#btn_update').click(function() {
        $.post('<?= base_url('billing/case/update') ?>', $('form.form1').serialize(), function(data) {
            if (data.update == 0) {
    ...
    $('#btn_lab').click(function() {
        var c_id = $('#c_id').val();
        load_form('<?= base_url('billing/case/addPathTest') ?>/' + c_id);
    });
