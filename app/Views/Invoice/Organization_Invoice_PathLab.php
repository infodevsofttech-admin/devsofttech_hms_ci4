...
<script>
    $(document).ready(function(){
		$("#itype_idv").change(function(){
			...
			$.post('<?= base_url('OcasePathLap/list_pathtest_bytype') ?>',{ "itype_idv": $('#itype_idv').val(),"ins_id": $('#ins_id').val(), [csrf_name]: csrf_value }, function(data){
		...
		$("#input_charge_code").change(function(){
			var csrf_name = '<?= csrf_token() ?>';
			var csrf_value = $('input[name="<?= csrf_token() ?>"]').first().val() || '<?= csrf_hash() ?>';
			$.post('<?= base_url('OcasePathLap/get_echs_id') ?>',{ "itype_idv": $('#itype_idv').val(),
			"input_charge_code": $('#input_charge_code').val(),"ins_id": $('#ins_id').val(), [csrf_name]: csrf_value }, function(data){
			if(data>0)
			{
		...
		$('#finalinvoice').click(function(){
			...
			$.post('<?= base_url('OcasePathLap/update_refer_doc') ?>',{ 
			"inv_id": inv_id, 
			"doc_id": doc_id,
			"refername":refername,
			...
			if(srno>0)
			{
				load_form('<?= base_url('OcasePathLap/showinvoice') ?>/'+inv_id);
			}else{
				alert('No Item Added');
			}
	...
	function add_item_invoice()
	{
		if($('#input_qty').val()>0)
		{
			var csrf_name = '<?= csrf_token() ?>';
			var csrf_value = $('input[name="<?= csrf_token() ?>"]').first().val() || '<?= csrf_hash() ?>';
			$.post('<?= base_url('OcasePathLap/showitem') ?>/1',{ "itype_name_id": $('#itype_name_id').val(), 
	...
	function remove_item_invoice(itemid)
	{
			var csrf_name = '<?= csrf_token() ?>';
			var csrf_value = $('input[name="<?= csrf_token() ?>"]').first().val() || '<?= csrf_hash() ?>';
			$.post('<?= base_url('OcasePathLap/showitem') ?>/0',{ "itemid": itemid, 
			"lab_invoice_id": $('#lab_invoice_id').val(),
			[csrf_name]: csrf_value }, function(data){
			$('#show_item_list').html(data);
			});
