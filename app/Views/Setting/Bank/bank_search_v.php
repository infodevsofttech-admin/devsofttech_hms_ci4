<div class="card">
    <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-2">
        <h3 class="card-title mb-0">Bank & Payment Sources</h3>
    </div>
    <div class="card-body">
        <?= csrf_field() ?>
        <div class="row g-3 align-items-end">
            <div class="col-lg-5">
                <label class="form-label">Bank (Charge Type)</label>
        ...
        <div class="row g-3">
            <div class="col-12">
                <div class="border rounded p-3 h-100">
                    <h6 class="mb-3">Payment Sources (Items)</h6>
...
<script>
    (function() {
        ...
        function loadSourcesForBank(bankId) {
            ...
            $.getJSON('<?= base_url('setting/admin/bank/sources') ?>/' + bankId, function(data) {
                updateCsrf(data);
        ...
        window.deleteBank = function(id) {
            ...
            $.post('<?= base_url('setting/admin/bank/delete') ?>', {
                "bank_id": id,
                ...
                load_form_div('<?= base_url('setting/admin/bank') ?>', 'maindiv', 'Bank & Payment Sources');
             }, 'json');
        };
        ...
        window.deleteSource = function(id) {
            ...
            $.post('<?= base_url('setting/admin/bank/source/delete') ?>', {
                "source_id": id,
                [csrf.name]: csrf.value
            }, function(data) {
                updateCsrf(data);
        ...
        $('#btn_modal_save').click(function() {
            ...
            var url = bankId ? '<?= base_url('setting/admin/bank/update') ?>' : '<?= base_url('setting/admin/bank/create-with-source') ?>';
            var payload = {
        ...
        $('#btn_modal_source_save').click(function() {
            ...
            var url = sourceId ? '<?= base_url('setting/admin/bank/source/update') ?>' : '<?= base_url('setting/admin/bank/source/create') ?>';
            var payload = {
