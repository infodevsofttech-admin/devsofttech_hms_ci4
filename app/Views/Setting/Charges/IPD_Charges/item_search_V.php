<div class="card">
    <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-2">
        <h3 class="card-title mb-0">IPD Charges</h3>
        <div class="card-tools ms-auto">
            <button class="btn btn-light" type="button" onclick="load_form_div('<?= base_url('item-ipd/search-itemtype') ?>','maindiv','IPD Charge Groups');">
    ...
    <div class="card-body">
        ...
        <div class="row mt-3">
            <div class="col-12">
                <div id="search_div">
                    <?= view('Setting/Charges/IPD_Charges/item_search_adv', ['data' => $data, 'typeId' => $typeId]) ?>
                </div>
            </div>
        </div>
    </div>
...
<script>
    (function() {
        var searchAdvUrl = '<?= base_url('item-ipd/search-adv') ?>';
        var printUrl = '<?= base_url('item-ipd/search-print') ?>';
        var exportUrl = '<?= base_url('item-ipd/export-excel') ?>';
        ...
        $('#tallModal_item').on('shown.bs.modal', function(event) {
            ...
            $.get('<?= base_url('item-ipd/item-record') ?>/' + testid, function(data) {
