<div class="card">
    <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-2">
        <h3 class="card-title mb-0">IPD Charges Group - Add New</h3>
    ...
    <div class="card-body">
        ...
        <form action="<?= base_url('item-ipd/create-type') ?>" method="post" role="form" class="form1">
            <?= csrf_field() ?>
            <div class="row g-3">
                <div class="col-md-4">
...
<script>
    $(document).ready(function() {
        $('form.form1').on('submit', function(form) {
            form.preventDefault();
            $.post('<?= base_url('item-ipd/create-type') ?>', $('form.form1').serialize(), function(data) {
                if (data.insertid == 0) {
                    if (typeof notify === 'function') {
                        notify('error', 'Please Attention', data.error_text || 'Please Check');
                    }
                } else {
                    load_form_div('<?= base_url('item-ipd/itemtype-record') ?>/' + data.insertid, 'maindiv');
                }
                updateCsrf(data);
            }, 'json');
        });
