<?php if (! empty($data_item)) : ?>
    <div class="card">
        <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-2">
            <h3 class="card-title mb-0">OPD Charge Name: <em><?= esc($data_item[0]->idesc ?? '') ?></em></h3>
        </div>
        <div class="card-body">
            <div class="jsError"></div>
            <form action="<?= base_url('item/update') ?>" method="post" role="form" class="form1">
                <?= csrf_field() ?>
                    ...
                    <div class="col-md-3">
                        ...
                        <select class="form-select" name="Item_Type" id="Item_Type">
                            <?php foreach ($data_item_type as $row) : ?>
                                <option value="<?= esc($row->itype_id ?? '') ?>" <?= combo_checked($data_item[0]->itype_id ?? '', $row->itype_id ?? '') ?>>
                                    <?= esc($row->group_desc ?? '') ?>
                                </option>
                            <?php endforeach ?>
                        </select>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div id="incomplist">
                        <?= view('Setting/Charges/OPD_Charges/item_insurance_list', ['data_insurance_item' => $data_insurance_item]) ?>
                    </div>
                    <hr/>
    ...
    <script>
        $(document).ready(function() {
            $('#btn_update').click(function() {
                $.post('<?= base_url('item/update') ?>', $('form.form1').serialize(), function(data) {
                    if (data.update == 0) {
        ...
        function add_item_spec() {
            ...
            $.post('<?= base_url('item/insurance/add') ?>', {
        ...
        function remove_item_spec(item_id) {
            var csrf = getCsrfPair();
            $.post('<?= base_url('item/insurance/remove') ?>', {
