<div class="card">
    <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-2">
        <h3 class="card-title mb-0">IPD Package - Add New</h3>
        <div class="card-tools ms-auto">
            <button class="btn btn-light" type="button" onclick="load_form_div('<?= base_url('package/search') ?>','maindiv','IPD Package');">
    ...
    <div class="card-body">
        ...
        <form action="<?= base_url('package/create') ?>" method="post" role="form" class="form1">
            <?= csrf_field() ?>
            <input type="hidden" value="0" id="p_id" name="p_id" />
            <div class="row g-3">
                <div class="col-md-4">
                    <label class="form-label">Package Name</label>
                    <input class="form-control" name="input_Item_name" placeholder="Package Name" value="" type="text" autocomplete="off">
                </div>
                <div class="col-md-4">
                    ...
                    <select class="form-select" name="Item_Type" id="Item_Type">
                        <?php foreach ($data_item_type as $row) : ?>
                            <option value="<?= esc($row->pak_id ?? '') ?>"><?= esc($row->pakage_group_name ?? '') ?></option>
                        <?php endforeach ?>
                    </select>
                </div>
                <div class="col-md-4">
...
<script>
    $(document).ready(function() {
        $('form.form1').on('submit', function(form) {
            form.preventDefault();
            $.post('<?= base_url('package/create') ?>', $('form.form1').serialize(), function(data) {
                if (data.insertid == 0) {
                    if (typeof notify === 'function') {
                        notify('error', 'Please Attention', data.error_text || 'Please Check');
                    }
                } else {
                    load_form_div('<?= base_url('package/item-record') ?>/' + data.insertid, 'maindiv');
                }
                updateCsrf(data);
            }, 'json');
        });
