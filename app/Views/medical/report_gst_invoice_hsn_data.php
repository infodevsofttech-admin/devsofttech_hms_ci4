...
<div class="table-responsive">
    <table class="table table-bordered table-striped table-sm align-middle" id="gst-hsn-list-table">
        <thead>
        <tr>
            <th>HSN Code</th>
            <th class="text-end">Item Qty</th>
            <th class="text-end">Tax Per.</th>
        ...
        <?php if (!empty($rows)): ?>
            <?php foreach ($rows as $row): ?>
                <tr>
                    <td><?= esc((string) ($row['hsn_code'] ?? '-')) ?></td>
                    <td class="text-end"><?= esc(number_format((float) ($row['no_of_qty'] ?? 0), 2)) ?></td>
                    <td class="text-end"><?= esc(number_format((float) ($row['cgst_per'] ?? 0), 2)) ?></td>
