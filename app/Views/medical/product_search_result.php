    ...
    <table id="product_report_list" class="table table-bordered table-striped table-sm align-middle">
        ...
        <?php foreach (($product_list ?? []) as $row): ?>
            <tr>
                ...
                <td>
                    <button onclick="openDrugMasterSubView('<?= base_url('Product_master/Product_edit/' . (int) ($row->id ?? 0)) ?>','Drug Master : Edit :Pharmacy');" type="button" class="btn btn-warning btn-sm">Edit</button>
                </td>
            </tr>
        <?php endforeach; ?>
        </tbody>
