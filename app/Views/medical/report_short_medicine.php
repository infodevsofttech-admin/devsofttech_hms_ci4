        ...
        <div class="card-body pt-3">
            <div class="row g-3 align-items-end">
                ...
                <div class="col-md-4">
                    ...
                    <select class="form-select" id="schedule_id" multiple size="5">
                        <?php foreach (($med_formulation ?? []) as $row): ?>
                            <option value="<?= (int) ($row->id ?? 0) ?>"><?= esc((string) ($row->formulation_length ?? ($row->formulation ?? ('Formulation-' . ($row->id ?? ''))))) ?></option>
                        <?php endforeach; ?>
                    </select>
                    <div class="form-text">Blank = All formulations</div>
                </div>
                <div class="col-md-2">
                    <div class="btn-group">
...
(function () {
    ...
    function selectedFormulationParam() {
        var sel = document.getElementById('schedule_id');
        var values = [];
    ...
    function buildUrl(output) {
        ...
        var formulation = selectedFormulationParam();
        return '<?= base_url('Medical_Report/Short_medicine_data') ?>/' + document.getElementById('opd_date_range').value + '/' + formulation + '/' + output;
    }
