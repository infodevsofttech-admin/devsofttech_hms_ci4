        ...
        <div class="report-split-left">
            <div class="card admin-card">
                <div class="card-header bg-white d-flex align-items-center justify-content-between">
                    ...
                    <button onclick="load_form_div('<?= base_url('Lab_Admin/reportedit_ultrasound_load') ?>/<?= esc($modality ?? 2) ?>/0','test_div');" type="button" class="btn btn-primary btn-sm">Add New Report</button>
                </div>
                <div class="card-body" style="height:500px;overflow-y:auto;">
                    <table id="report_list" class="table table-striped table-hover align-middle TableData">
                        ...
                        <?php for ($i = 0; $i < count($labReport_master); ++$i) { ?>
                            <tr>
                                <td><?= esc($labReport_master[$i]->template_name ?? '') ?></td>
                                <td><?= esc($labReport_master[$i]->title ?? '') ?></td>
                                <td>
                                    <button onclick="load_form_div('<?= base_url('Lab_Admin/reportedit_ultrasound_load') ?>/<?= esc($modality ?? 2) ?>/<?= esc($labReport_master[$i]->id ?? 0) ?>','test_div');" type="button" class="btn btn-outline-secondary btn-sm">Edit</button>
                                </td>
                            </tr>
                        <?php } ?>
                        </tbody>
