    ...
    <form class="form1">
        ...
        <div class="row g-3 rx-two-panel">
            <div class="col-lg-4 rx-left-panel">
                ...
                <div class="card mt-3">
                    <div class="card-header"><strong>Old Consult (Fast View)</strong></div>
                    <div class="card-body">
                        <div class="mb-3">
                            <label class="form-label mb-1"><strong>Current OPD Uploaded Scans</strong></label>
                            <?php if (!empty($current_opd_scans ?? [])) { ?>
                                <div class="row g-2">
                                    <?php foreach (($current_opd_scans ?? []) as $scan) { ?>
                                        <div class="col-12">
                                            <div class="border rounded p-2">
                                                <div class="d-flex gap-2">
                                                    <div style="width:84px;flex:0 0 84px;">
                                                        <?php if (!empty($scan['is_pdf'])) { ?>
                                                            <a class="btn btn-outline-secondary btn-sm w-100" target="_blank" href="<?= esc((string) ($scan['path'] ?? '')) ?>">PDF</a>
                                                        <?php } else { ?>
                                                            <a target="_blank" href="<?= esc((string) ($scan['path'] ?? '')) ?>">
                                                                <img src="<?= esc((string) ($scan['path'] ?? '')) ?>" loading="lazy" style="width:84px;height:72px;object-fit:cover;border:1px solid #dfe3e7;border-radius:4px;" alt="scan">
                                                            </a>
                                                        <?php } ?>
                                                    </div>
                                                    <div class="flex-grow-1 small">
                                                        <div class="mb-1">
                                                            <span class="badge bg-light text-dark border"><?= esc((string) ($scan['document_type'] ?? $scan['scan_type'] ?? 'General')) ?></span>
                                                            <?php $status = strtolower((string) ($scan['ai_status'] ?? '')); ?>
                                                            <?php if ($status === 'completed') { ?><span class="badge bg-success">AI Ready</span><?php } ?>
                                                            <?php if ($status === 'failed') { ?><span class="badge bg-danger">AI Failed</span><?php } ?>
                                                            <?php if ((int) ($scan['ai_alert_flag'] ?? 0) === 1) { ?><span class="badge bg-danger">Alert</span><?php } ?>
                                                        </div>
                                                        <?php if (!empty($scan['report_text'])) { ?>
                                                            <div class="text-muted mb-1" style="white-space:pre-line;line-height:1.25;"><?= esc((string) ($scan['report_text'] ?? '')) ?></div>
                                                            <button type="button" class="btn btn-sm btn-outline-primary btn-use-current-scan-report" data-report="<?= esc((string) ($scan['report_text'] ?? '')) ?>">Use Report</button>
                                                        <?php } else { ?>
                                                            <div class="text-muted">Report details are not available yet.</div>
                                                        <?php } ?>
                                                        <div class="text-muted mt-1"><?= esc((string) ($scan['insert_date'] ?? '')) ?></div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <?php } else { ?>
                                <div class="text-muted">No uploaded scan found for this OPD yet.</div>
                            <?php } ?>
                        </div>

                        <?php if (!empty($old_rx_fast ?? [])) { ?>
                            <?php $renderedOldFastRows = 0; ?>
                            <?php foreach (($old_rx_fast ?? []) as $oldRow) { ?>
                                <?php
                                    $oldComplaints = trim((string) ($oldRow['complaints'] ?? ''));
                                    $oldDiagnosis = trim((string) ($oldRow['diagnosis'] ?? ''));
                                    $oldScans = $oldRow['scans'] ?? [];
                                    $oldMedicines = $oldRow['medicines'] ?? [];
                                    $hasOldData = ($oldComplaints !== '' || $oldDiagnosis !== '' || !empty($oldScans) || !empty($oldMedicines));
                                ?>
                                <?php if (!$hasOldData) { continue; } ?>
                                <?php $renderedOldFastRows++; ?>
                                <div class="border rounded p-2 mb-2 old-rx-card">
                                    <div class="d-flex justify-content-between align-items-center gap-2 flex-wrap">
                                        <div><strong><?= esc($oldRow['opd_code'] ?? '') ?></strong> <small class="text-muted"><?= esc($oldRow['opd_date'] ?? '') ?></small></div>
                                        <?php if (!empty($oldMedicines)) { ?>
                                            <button type="button" class="btn btn-sm btn-outline-success btn-old-rx-add-all">+ All</button>
                                        <?php } ?>
                                    </div>
                                    <?php if ($oldComplaints !== '') { ?>
                                        <div class="small"><strong>C:</strong> <?= esc($oldComplaints) ?></div>
                                    <?php } ?>
                                    <?php if ($oldDiagnosis !== '') { ?>
                                        <div class="small"><strong>D:</strong> <?= esc($oldDiagnosis) ?></div>
                                    <?php } ?>
                                    <?php if (!empty($oldMedicines)) { ?>
                                        <div class="table-responsive mt-2">
                                            <table class="table table-bordered table-sm mb-1">
                                                ...
                                                <tbody>
                                                    <?php foreach ($oldMedicines as $indexMed => $oldMed) { ?>
                                                        <tr>
                                                            <td><?= (int) $indexMed + 1 ?></td>
                                                            <td><?= esc((string) ($oldMed['med_name'] ?? '')) ?></td>
                                                            <td>
                                                                <button
                                                                    type="button"
                                                                    class="btn btn-sm btn-outline-primary btn-old-rx-add-one px-2"
                                                                    title="Add medicine"
                                                                    data-med-id="<?= (int) ($oldMed['med_id'] ?? 0) ?>"
                                                                    data-med-name="<?= esc((string) ($oldMed['med_name'] ?? '')) ?>"
                                                                    data-med-type="<?= esc((string) ($oldMed['med_type'] ?? '')) ?>"
                                                                    data-dosage="<?= esc((string) ($oldMed['dosage'] ?? '')) ?>"
                                                                    data-dosage-when="<?= esc((string) ($oldMed['dosage_when'] ?? '')) ?>"
                                                                    data-dosage-freq="<?= esc((string) ($oldMed['dosage_freq'] ?? '')) ?>"
                                                                    data-no-of-days="<?= esc((string) ($oldMed['no_of_days'] ?? '')) ?>"
                                                                    data-qty="<?= esc((string) ($oldMed['qty'] ?? '')) ?>"
                                                                    data-remark="<?= esc((string) ($oldMed['remark'] ?? '')) ?>"
                                                                >
                                                                    +
                                                                </button>
                                                            </td>
                                    ...
                                    <?php } ?>
                                    <?php if (!empty($oldScans)) { ?>
                                        <div class="mt-2 d-flex flex-wrap gap-2">
                                            <?php foreach ($oldScans as $scan) { ?>
                                                <?php if (!empty($scan['is_pdf'])) { ?>
                                                    <a class="btn btn-sm btn-outline-secondary" target="_blank" href="<?= esc($scan['path']) ?>">Open PDF</a>
                                                <?php } else { ?>
                                                    <img src="<?= esc($scan['path']) ?>" loading="lazy" style="width:70px;height:70px;object-fit:cover;border:1px solid #dfe3e7;border-radius:4px;" alt="scan">
                                                    <button type="button" class="btn btn-sm btn-outline-primary btn-scan-extract" data-file="<?= esc($scan['path']) ?>" data-type="<?= esc($scan['scan_type'] ?? 'general') ?>">Text</button>
                                                <?php } ?>
                                            <?php } ?>
                                        </div>
                                    <?php } ?>
                            ...
                            <?php } ?>
                            <?php if ($renderedOldFastRows === 0) { ?>
                                <div class="text-muted">No old consult record found.</div>
                            <?php } ?>
                        <?php } else { ?>
                            <div class="text-muted">No old consult record found.</div>
                        <?php } ?>
                        ...
                        <div class="mt-2">
                            <label class="form-label mb-1"><strong>Extracted Scan Text</strong></label>
...
(function() {
    ...
    $('#med_name').on('input', function() {
        ...
        apiGet('<?= base_url('Opd_prescription/medicine_search') ?>?q=' + encodeURIComponent(q) + '&scope=' + encodeURIComponent(activeMedicineScope), function(data) {
            ...
            medicineSuggestRows.forEach(function(row) {
                var label = (row.med_name || '').toString();
                if ((parseInt(row.is_favorite || 0, 10) === 1)) {
                    label = '★ ' + label;
                }
                if (parseInt(row.use_count || 0, 10) > 0) {
                    label += ' (' + parseInt(row.use_count || 0, 10) + ')';
                }
    ...
    $('#med_name').on('change', function() {
        var val = $(this).val();
        var matched = null;
        $('#medicine_suggest option').each(function() {
            if ($(this).val() === val) {
                matched = $(this);
                return false;
            }
        ...
        if (!matched) {
            var normalizedInput = val.replace(/^★\s*/, '').replace(/\s*\(\d+\)\s*$/, '').trim();
            $('#medicine_suggest option').each(function() {
                if (String($(this).data('name') || '').trim().toUpperCase() === normalizedInput.toUpperCase()) {
                    matched = $(this);
                    return false;
                }
            });
        ...
        if (matched) {
            var medName = String(matched.data('name') || '').trim();
            if (medName) {
                $('#med_name').val(medName);
            }

            if (!$('#med_type').val()) {
                $('#med_type').val(matched.data('type') || '');
            }

            var medId = parseInt(matched.data('id') || '0', 10);
            var isFav = parseInt(matched.data('fav') || '0', 10) === 1;
            if (medId > 0) {
