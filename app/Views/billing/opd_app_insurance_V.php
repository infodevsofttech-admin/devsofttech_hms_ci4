            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <?php if (empty($insurance_id)) : ?>
                        <div class="alert alert-warning">No insurance found for this patient.</div>
                    <?php endif; ?>

                    <form class="form1" role="form">
                        <?= csrf_field() ?>
                        <input type="hidden" id="pid" value="<?= esc($person_info[0]->id ?? 0) ?>" />
                        <input type="hidden" id="insurance_id" value="<?= esc($insurance_id ?? 0) ?>" />
                        ...
                        <div class="row g-3 mt-2" id="ShowDoctor">
                            <div class="col-md-8">
                                ...
                                <button type="button" class="btn btn-primary mt-2" id="btnnextfee" <?= empty($insurance_id) ? 'disabled' : '' ?>>Next to Fee</button>
                            </div>
                        </div>
...
<script>
    (function() {
        ...
        $('#btnnextfee').click(function() {
            ...
            if (!insuranceId) {
                alert('No insurance available for this patient.');
                return;
            }

            var csrf = getCsrfPair();
            $('#showfee').show();
            $.post('<?= base_url('Opdcase/showfee') ?>', {
                "doc_id": checkValue,
        ...
        $('#btnnextconfirm').click(function() {
            ...
            var csrf = getCsrfPair();
            $.post('<?= base_url('Opdcase/confirm_opd') ?>', {
