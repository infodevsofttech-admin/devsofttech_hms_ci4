   ...
   class Medical_M extends CI_Model {
		...
		public function add_invoiceitem($data)
		{
			if ($this->db->insert("inv_med_item", $data)) { 
				...
				if($insert_id>0)
				{
					$this->db->set("total_sale_unit","total_sale_unit+".$data["qty"],false); 
					$this->db->where("id", $data["store_stock_id"]); 
		...
		{
			$sql="select * from inv_med_item where id=".$old_id_no;
			$query = $this->db->query($sql);
			$result	= $query->result();
			
			$old_qty=$result[0]->qty;
			$diff_qty=$data["qty"]-$old_qty;
			$store_stock_id=$result[0]->store_stock_id;
			...
			{
				$this->db->set("total_sale_unit","total_sale_unit+".$diff_qty,false); 
				$this->db->where("id", $store_stock_id); 
				$this->db->update("purchase_invoice_item");
			}
		}
		
		public function delete_invoiceitem($itemid)
		{
			...
			if($this->db->delete("inv_med_item", "id=".$itemid))
			{
				$this->db->set("total_sale_unit","total_sale_unit-".$old_qty,false); 
				$this->db->where("id", $store_stock_id); 
				$this->db->update("purchase_invoice_item");
			}
		}
	  ...
	  public function insert_payment_history_log($data) { 
		...
		public function update_purchase_item_stock_adjust($data_stock_adjuct,$item_id) { 
			$sql="select sum(qty) as total_adjust from purchase_item_stock_adjust where item_id=".$item_id;
			$query = $this->db->query($sql);
			$result	= $query->result();
			
			$sql="select *,(total_unit-total_return_unit-total_lost_unit-total_sale_unit) as Cur_unit_qty
