		...
		<div id="supplier_list" class="box-body" style="height:500px;overflow-y:auto;">
		  <table id="report_list" class="table table-bordered table-striped TableData">
			...
			<?php for ($i = 0; $i < count($purchase_list); ++$i) { 
				if($purchase_list[$i]->ischallan==0){
					echo '<tr>';
				}else{
			...
			?>
			  ...
			  <td>
				  	<button onclick="load_form_div('/Storestock/PurchaseMasterEdit/<?=$purchase_list[$i]->id ?>','searchresult');" type="button" class="btn btn-warning">View & Edit</button>
					  <a href="<?php echo '/Storestock/print_purchase/'.$purchase_list[$i]->id;  ?>" target="_blank" class="btn btn-default"><i class="fa fa-print"></i> Print</a>
		 ...
		 <script>
			...
			function delete_invoice(inv_id)
			{
				...
				{
					$.post('/Storestock/purchase_invoice_delete/'+inv_id, $('form.form2').serialize(), function(data){
					if(data.is_delete==0)
					{
