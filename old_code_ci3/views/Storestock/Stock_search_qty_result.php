					...
					<?php foreach ($stock_list as $row) { ?>
						<tr>
							...
							<button type="button" class="btn btn-primary" 
								...
								data-product_pak="<?=$row->packing ?>"
								data-etype="2"
								data-productname='<?=$row->item_name ?>'>Show</button>
							</td>
						</tr>
					<?php } ?>
				</tbody>
...
<script>
	function update_sale_stock(item_id)
	{
		...
		{
		$.post('/index.php/Storestock/update_stock_adjust/',
	...
	function remove_sale_stock(item_id)
	{
		...
		{
		$.post('/index.php/Storestock/remove_stock_item/',
			{ "item_id":item_id,
				'<?=$this->security->get_csrf_token_name()?>':csrf_value
			 }, function(data){
	...
	function add_sale_stock(item_id)
	{
		...
		{
		$.post('/index.php/Storestock/add_stock_item/',
			{ "item_id":item_id,
				'<?=$this->security->get_csrf_token_name()?>':csrf_value
			 }, function(data){
				notify('success','Please Attention',data);
	...
	$('#tallModal').on('shown.bs.modal', function (event) {
		...
		if(etype=='1')
		{
			$.post('/index.php/Storestock/get_product_stock/'+product_id,
		...
		if(etype=='2')
		{
			$.post('/index.php/Storestock/get_product_stock_removed/'+product_id,
			{ 	"product_id": product_id,
				"productname": productname,
				"date_range":"0",
                "supplier_id":"0",
