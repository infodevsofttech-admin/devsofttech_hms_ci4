<section class="content-header">
    <h1>
        IPD Charge Name : <em><?=$data_item[0]->idesc; ?></em>
    </h1>
</section>
<!-- Main content -->
							...
							<div class="col-md-3">
								<div class="form-group">
									...
									<?php 
										...
										{ 
											echo '<option value="'.$row->itype_id.'"  '.combo_checked($data_item[0]->itype,$row->itype_id).'  >'.$row->group_desc.'</option>';
										}
									?>   
									</select>
								</div>
...
<script>
    $(document).ready(function(){
        $('#btn_update').click(function(){
			$.post('/index.php/Item_IPD/UpdateRecord', 
   ...
   function add_item_spec()
		{
			...
			$.post('/index.php/item_IPD/AddInsuranceItemRecord',
		...
		function remove_item_spec(item_id)
		{
			...
			$.post('/index.php/item_IPD/remove_record_item',
			{ "in_remove_id": item_id,"p_id": $('#p_id').val(),"isadd":0,
				"<?=$this->security->get_csrf_token_name()?>":csrf_value
			}, function(data){
			$('#incomplist').html(data);
			});
