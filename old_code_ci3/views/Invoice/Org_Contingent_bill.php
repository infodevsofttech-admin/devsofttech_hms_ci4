...
$('#updatereport').click(function(){
			...
			$.post('/index.php/Orgcase/org_contingent_update',{ 
					"case_id": caseid, 
					"HTMLData":HTMLData,
		...
		$('#createreport').click(function(){
			...
			{
			$.post('/index.php/Orgcase/create_contingent_bill/'+caseid,{ 
					"case_id": caseid,
					'<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>'
					}, function(data){
						notify('success','Please Attention',data);
						load_form_div('/Orgcase/contingent_bill/'+caseid,'org_CONTINGENT');
				});
			}
		...
		$('#showreport').click(function(){
			var caseid = $('#caseid').val();
           	load_report_div('/Orgcase/contingent_bill/'+caseid+'/1','org_CONTINGENT');
        });
		
		$('#editreport').click(function(){
			var caseid = $('#caseid').val();
           	load_form_div('/Orgcase/contingent_bill/'+caseid+'/0','org_CONTINGENT');
        });
