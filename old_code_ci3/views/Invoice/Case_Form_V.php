<section class="content-header">
    ...
    <ol class="breadcrumb">
        <li><a href="javascript:load_form('/Patient/person_record/<?=$data[0]->id ?>');"><i class="fa fa-dashboard"></i> Person</a></li>
      ...
      <div class="row">
        <div class="col-md-12">
            <?php echo form_open('Patient/create', array('role'=>'form','class'=>'form1')); ?>
						...
						<input type="hidden" value="<?=$hc_insurance_card[0]->id ?>" id="inc_card_id" name="inc_card_id" />
						<input type="hidden" value="<?=$data_insurance[0]->active ?>" id="insurance_active" name="insurance_active" />
                        <div class="row">
							<div class="col-md-4">
						...
						<div class="row">
							...
							<div class="col-md-4"> 
								<div class="form-group">
									...
									<input class="form-control" name="input_insurance_id" placeholder="Insurance Number" type="text" value="<?=$hc_insurance_card[0]->insurance_no ?>"  >
								</div>
							</div>
							<div class="col-md-4"> 
								<div class="form-group">
									<label>Insurance Card Holder Name</label>
									<input class="form-control" name="input_card_holder_name" placeholder="Name of Card Holder" type="text" value="<?=$hc_insurance_card[0]->card_holder_name ?>" >
								</div>
							</div>
						</div>
						<div class="row">
...
<script>
   $(document).ready(function(){
        $('form.form1').on('submit', function(form){
			...
			{
				$.post('/index.php/Ocasemaster/create', $('form.form1').serialize(), function(data){
					if(data.insertid==0)
					{
						$('div.jsError').html(data.error_text);
					}else
					{
						load_form('/Ocasemaster/open_case/'+data.insertid+'/'+case_type);
					}
            	}, 'json');
			}else{
