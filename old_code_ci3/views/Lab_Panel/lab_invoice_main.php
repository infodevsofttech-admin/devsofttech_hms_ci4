        ...
        <div class="col-md-6">
				...
				<input type="hidden" value="<?=$data[0]->id ?>" id="p_id" name="p_id" />
                        <div class="row">
							...
							<div class="col-md-6">
								...
								<span class="text-primary"> / Age : </span> <span class="text-success"><?=$data[0]->str_age?> </span></p>
							</div>
						</div>
						<div class="row">
							<div class="col-md-9">
								<p class="text-primary">Relation : <span class="text-success"><?=$data[0]->p_relative?></span> 								
								<span class="text-primary">Relative Name :</span> <span class="text-success"><?=ucwords($data[0]->p_rname)?></span></p>
							</div>
							<div class="col-md-3">
			...
			<div class="row">
				<div class="col-md-12">
					...
					accesskey="C" onclick="report_compile('<?=$inv_id ?>','<?=$lab_type ?>')" >
					Compile for Combine</button>
					
					<!--
					<button type="button" class="btn btn-danger" id="btn_lab" 
					data-toggle="modal"
					data-target="#tallModal_4" data-testid="<?=$inv_id ?>" 
					data-labtype="<?=$lab_type ?>" 
					data-testname="<?=$inv_code ?>" data-etype="1"
					...
					data-target="#tallModal_4" data-testid="<?=$inv_id ?>" 
					data-labtype="<?=$lab_type ?>"
					data-testname="<?=$inv_code ?>" data-etype="2"
					accesskey="R">Report in LetterHead</button>

					<button type="button" class="btn btn-warning" id="btn_ipd" 
					data-toggle="modal"
					data-target="#tallModal_4" data-testid="<?=$inv_id ?>" 
					data-labtype="<?=$lab_type ?>"
					data-testname="<?=$inv_code ?>" data-etype="9"
					accesskey="R">Report In Plain Paper</button>
			...
			<div class="row">
				<div class="col-md-12">
					<button type="button" class="btn btn-info" id="btn_ipd" 
					data-toggle="modal"
					data-target="#tallModal_4" data-inv_id="<?=$inv_id ?>" 
					data-labtype="<?=$lab_type ?>"
					data-testname="<?=$inv_code ?>" data-etype="3"
					accesskey="U">Upload Complete Report(In PDF)</button>


					<button type="button" class="btn btn-success" id="btn_ipd" 
					data-toggle="modal"
					data-target="#tallModal_4" data-inv_id="<?=$inv_id ?>" 
					data-labtype="<?=$lab_type ?>"
					data-testname="<?=$inv_code ?>" data-etype="4"
					accesskey="S">Scan Complete Report</button>

					
					<button type="button" class="btn btn-default" id="btn_ipd" data-toggle="modal"
					data-target="#tallModal_4" data-inv_id="<?=$inv_id ?>" 
					data-labtype="<?=$lab_type ?>"
					data-testname="<?=$inv_code ?>" data-etype="5"
...
<script>
   ...
   function update_request(test_id)
	...
	$('#tallModal').on('shown.bs.modal', function (event) {
		...
		if(etype=='1')
		{
			$.post('/index.php/Lab_Admin/lab_tab_2_process/'+testid,
			{ 	"test_id": testid,
		...
		if(etype=='2')
		{
			...
			$.post('/index.php/Lab_Admin/lab_file_scan/'+repoid+'/'+testid,
			{ 	"test_id": testid,
				'<?=$this->security->get_csrf_token_name()?>':csrf_value }, function(data){
			$('#testentry-bodyc').html(data);
			});
		...
		{
			var repoid = button.data('repoid');
			$.post('/index.php/Lab_Admin/lab_file_scan/'+repoid+'/'+testid+'/1',
			{ 	"test_id": testid,
				'<?=$this->security->get_csrf_token_name()?>':csrf_value }, function(data){
			$('#testentry-bodyc').html(data);
			});
		...
		{
			var repoid = button.data('repoid');
			$.post('/index.php/Lab_Admin/lab_file_upload/'+repoid+'/'+testid,
			{ 	"test_id": testid,
				'<?=$this->security->get_csrf_token_name()?>':csrf_value }, function(data){
			$('#testentry-bodyc').html(data);
			});
		...
		{
			var repoid = button.data('repoid');
			$.post('/index.php/Lab_Admin/lab_file_list/'+repoid+'/'+testid,
			{ 	"test_id": testid,
				'<?=$this->security->get_csrf_token_name()?>':csrf_value }, function(data){
			$('#testentry-bodyc').html(data);
			});
		...
		if(etype=='6')
		{
			...
			var Get_Query='/index.php/Lab_Admin/report_compile_single/'+inv_id+'/'+lab_type+'/'+repoid+'/1';
		...
		if(etype=='7')
		{
			...
			$.post('/index.php/Lab_Admin/show_report_final/'+repoid+'',
			{ 	"test_id": repoid,
				'<?=$this->security->get_csrf_token_name()?>':csrf_value }, function(data){
			$('#testentry-bodyc').html(data);
			});
