...
<script type="text/javascript" language="javascript" >
			$(document).ready(function() {
				...
				var dataTable = $('#employee-grid').DataTable( {
					...
					"ajax":{
						url :"Medical_backpanel/getInvoiceTable", // json datasource
						...
						render: function ( data, type, row, meta ) {
							if(type === 'display'){
								if((row[8]>0 && row[10]>0) || (row[9]>0 && row[9]>11))
								{
									...
									url_link = "javascript:load_form_div('/Medical_backpanel/Invoice_med_show/"+ encodeURIComponent(row[7]) + "','maindiv');" ;
				...
				$('#employee-grid tbody').on( 'click', 'button', function () {
					...
					load_form_div('/Medical_backpanel/Invoice_med_show/'+data[7],'maindiv');
				} );
