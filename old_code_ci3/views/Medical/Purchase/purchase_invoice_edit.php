...
<script>
	...
	function show_challan() {
		$('#purchase_items_old_history').show();
		load_form_div('/Medical/challan_invoice/<?= $purchase_invoice[0]->sid ?>', 'purchase_items_old_history');
	}

	function add_item_in_invoice(ss_no) {
		$.post('/index.php/Medical/challan_item_to_purchase/' + ss_no + '/<?= $purchase_invoice[0]->id ?>', $('form.form1').serialize(), function(data) {
	...
	function return_to_challan(ss_no) {
		$.post('/index.php/Medical/challan_item_return/' + ss_no, $('form.form1').serialize(), function(data) {
			if (data.is_transfer == 0) {
				notify('error', 'Please Attention', data.show_text);
			} else {
				notify('success', 'Please Attention', data.show_text);
