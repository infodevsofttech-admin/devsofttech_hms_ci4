            ...
            <div class="box-body">
                ...
                <div class="row">
                        ...
                        <div class="form-group">
                            <select name="input_formulation" id="input_formulation" class="form-control">
                                <?php 
									...
									{
										$selected = ($row->formulation == $product_data[0]->formulation) ? ' selected="selected"' : "";

										echo '<option value="'.$row->formulation.'"  '.$selected .' >'.$row->formulation_length.'</option>';
									} 
									?>
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            <div class="ui-widget">
                                ...
                                <input class="form-control input-sm" name="input_genericname" id="input_genericname"
                                    placeholder="Generic Name" type="text" value="<?=$product_data[0]->genericname?>">
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6">
                    <label for="med_cat_id" class="control-label">Medicine Catagory</label>
                        <div class="form-group">
                            <select class="form-control select2" id=med_cat_id name="med_cat_id[]" multiple="multiple" data-placeholder="Select a Schedule, Blank for All"  >
                                <?php 
                                $cat_list=explode(",",$product_data[0]->cat_list);
                                print_r($cat_list);

                                foreach($med_product_cat_master as $row){ 
                                    
                                    if(in_array($row->id,$cat_list))
                                    {
                                        $selected=" Selected";
                                    }else{
                                        $selected="";
                                    ...
                                    ?>
                                    <option value='<?=$row->id?>' <?=$selected?> ><?=$row->med_cat_desc?></option>
                                <?php } ?>
                            </select>
                        </div>
                    </div>
                    ...
                    <?php 
							...
							$chk_schedule_x=$product_data[0]->schedule_x;
							if($chk_schedule_x==1)
							{
								$chk_schedule_x_checkbox_checked="checked";
							}else{
								$chk_schedule_x_checkbox_checked="";
							}


							$chk_schedule_g=$product_data[0]->schedule_g;
							if($chk_schedule_g==1)
							{
								$chk_schedule_g_checkbox_checked="checked";
							}else{
								$chk_schedule_g_checkbox_checked="";
							}

							$chk_high_risk=$product_data[0]->high_risk;
							if($chk_high_risk==1)
							{
								$chk_high_risk_checkbox_checked="checked";
							}else{
								$chk_high_risk_checkbox_checked="";
							}
...
$(document).ready(function() {
    ...
    $('#btn_update_stock').click(function() {
        $.post('/index.php/product_master/product_master_update/' + $('#product_id').val(), $(
