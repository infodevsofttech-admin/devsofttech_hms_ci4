...
.responsive {
  ...
  padding: 2px;
            ...
            <div class="box-body box-profile">
                <div class="col-xs-8">
                    ...
                    <a href="javascript:load_form_div('/Mobile_app/show_profile_opd/<?=$c->id?>','search_result')" class="btn btn-info btn-xs">
                        ...
                        <?php if($opd_current>0){?>
                        <a href="javascript:load_form_div('/Mobile_app/take_opd_picture/<?=$opd_current?>/<?=$c->id?>','search_result')" class="btn btn-info btn-xs">
                        <span class="fa fa-eye"></span>Scan [Today]</a>
                    <?php } ?>  
                </div>
                <div class="col-xs-4">
                    <?php
	...
	<?php 
            ...
            {
                $sql="select p.*,o.opd_code,o.doc_name,o.opd_id,date_format(o.apointment_date,'%d-%m-%Y') as str_date_opd_visit ,
                ...
                if(count($opd_data)>0)
                {
                    ...
                    echo '/<b>OPD Date :</b>'.$opd_data[0]->str_date_opd_visit.' /<b>Queue No.:</b>'.$opd_data[0]->queue_no.'<br/>';
		   ...
		   echo '<a href="javascript:load_form_div(\'/Mobile_app/take_opd_picture/'.$opd_data[0]->opd_id.'/'.$opd_data[0]->p_id.'\',\'search_result\')" class="btn btn-info btn-xs">
                        <span class="fa fa-eye"></span>Scan ['.$opd_data[0]->str_date_opd_visit.'] </a>';
                ...
                foreach($opd_file_list as $opd_file_row)
                {
                    ...
                    {
                        echo '<img id="img_opd_'.$opd_file_row->id.'" class="responsive"  src="'.$file_path.'"   
                            alt="Photo" data-toggle="modal" data-target="#myModal"
                            data-src="'.$file_path.'"
                            data-img_id="'.$opd_file_row->id.'" >';
                    }
                ...
                //OPD Movies Show
                foreach($file_opd_rec as $file_opd_rec_row)
                {
                    echo '<div class="row">
                            <div class="col-xs-12 col-sm-6 col-md-8 ">';
                    
                            $pos=strpos($file_opd_rec_row->full_path,'uploads/',1) ;
	                        $file_path=substr($file_opd_rec_row->full_path,$pos);
    

                    //$file_path=str_replace('hms_uploads','uploads',$file_opd_rec_row->full_path);
