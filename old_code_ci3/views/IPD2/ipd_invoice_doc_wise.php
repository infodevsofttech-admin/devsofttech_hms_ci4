...
<?php }else { echo '<b>Item Wise Bill</b><br>'; }  ?>
</h3>
</htmlpageheader>
...
<?php
$total_discount=$ipdmaster[0]->Discount+$ipdmaster[0]->Discount2+$ipdmaster[0]->Discount3;
$total_charge=$ipdmaster[0]->chargeamount1+$ipdmaster[0]->chargeamount2;
$gross_amount=$ipdmaster[0]->gross_amount-$ipdmaster[0]->package_med_amount-$ipdmaster[0]->med_amount;

$perdis=($total_discount*100)/$gross_amount;
...
<hr/>
<table  cellspacing="2"  style="font-size: 12px;width:100%;border-style: inset;" >
			<tr>
                ...
                <th style="width:100px;text-align:right">Amt After Discount</th>
			</tr>
			<?php
			$srno=1;
			$headdesc='';
			...
			if(Count($ipd_package)>0){
				...
				{ 
                    ...
                    echo '<td align="right">'.($ipd_package[$i]->package_Amount-($ipd_package[$i]->package_Amount*$perdis/100)).'</td>';
					$srno=$srno+1;
			...
			for($i=0;$i<Count($ipd_invoice_item);$i++)
				{ 
                    ...
                    echo '<td align="right">'.($ipd_invoice_item[$i]->item_amount-Round($ipd_invoice_item[$i]->item_amount*$perdis/100)).'</td>';
					$srno=$srno+1;
			...
			{ 
				...
				echo '<td align="right">'.($showinvoice[$i]->amount-Round($showinvoice[$i]->amount*$perdis/100)).'</td>';
				$srno=$srno+1;
