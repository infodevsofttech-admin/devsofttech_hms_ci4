				...
				foreach($inv_items as $row)
				{
					...
					elseif($row->id=='' && $row->inv_med_id == ''){
						...
						if($head_start==0)
						{
							...
							echo '<tr><td colspan="11" style="font-size=12px;"><b>Invoice ID : </b>'.$row->inv_med_code.' [<b>Dated : </b><i>'.$row->str_inv_date.'</i>]</td></tr>';
							echo '<tr>';
...
<?php if(count($inv_med_group)>0) {  ?>
		...
		<table style="font-size: 12px;width: 100%;border-width: 0.5px">
				...
				<tr>
					<th style="width: 10px">#</th>
					<th  align="center"><?=$inv_med_group[0]->discount_group?></th>
					<th  align="center"><?=$inv_med_group[0]->CGST_Tamount?></th>
					<th  align="center"><?=$inv_med_group[0]->SGST_Tamount?></th>
					<th  align="center"><?=$inv_med_group[0]->TaxableAmount?></th>
					<th  align="center"><?=number_format(round($inv_med_group[0]->net_amount),2)?></th>
					<?php //if($inv_med_group[0]->payment_balance>=0 ){ ?> 
					<th  align="center"><?=$inv_med_group[0]->payment_received?></th>
					<th  align="center"><?=$inv_med_group[0]->payment_balance?></th>
					<?php //} ?>
				</tr>
		</table>
		<hr/>	
