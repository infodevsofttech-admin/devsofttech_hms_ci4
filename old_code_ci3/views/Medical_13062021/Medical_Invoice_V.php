		...
		<div class="box-body">
			<div class="col-md-12" >
				...
				<div class="row " >
					...
					<div class="row">
						<div class="col-md-12">
							...
							<p class="text-red" >
								...
								<span class="text-lead" name="div_product_mrp" id="div_product_mrp">
								</span>
					...
					<div class="collapse" id="collapse_zero_stock">
						<div class="row" >
							<div class="col-md-2" style="padding: 3px;">
								<div class="form-group">
									<label>Batch No</label>
									<input class="form-control input-sm" name="input_Batch_no" id="input_Batch_no" placeholder="Batch No" autocomplete="off" />
								</div>
							</div>
							<div class="col-md-2" style="padding: 3px;">
								<div class="form-group">
									<label>Exp.Dt. </label>
									<input class="form-control varchar input-sm" name="input_ExpDt" id="input_ExpDt" placeholder="Expiry Date" type="text" value=0 autocomplete="off"  />
								</div>
							</div>
							<div class="col-md-2" style="padding: 3px;">
								<div class="form-group">
									<label>MRP</label>
									<input class="form-control number input-sm" name="input_MRP" id="input_MRP" placeholder="MRP" type="text" value=0 autocomplete="off"  />
								</div>
							</div>
							<div class="col-md-2" style="padding: 3px;">
								<div class="form-group">
									<label>Packing</label>
									<input class="form-control number input-sm" name="input_Packing" id="input_Packing" placeholder="Expiry Date" type="text" value=0 autocomplete="off"  />
								</div>
							</div>
							<div class="col-md-2" style="padding: 3px;">
								<div class="form-group">
									<label>CGST</label>
									<input class="form-control number input-sm" name="input_CGST" id="input_CGST" placeholder="CGST" type="text" value=0 autocomplete="off"  />
								</div>
							</div>
							<div class="col-md-2" style="padding: 3px;">
								<div class="form-group">
									<label>SGST</label>
									<input class="form-control number input-sm" name="input_SGST" id="input_SGST" placeholder="SGST" type="text" value=0 autocomplete="off"  />
								</div>
							</div>
							<div class="col-md-2" style="padding: 3px;">
								<div class="form-group">
									<label>HSN CODE</label>
									<input class="form-control number input-sm" name="input_HSNCODE" id="input_HSNCODE" placeholder="HSN CODE" type="text" value=0 autocomplete="off"  />
								</div>
							</div>
						</div>
					</div>
					<div class="row" >
						<div class="col-md-2" style="padding: 3px;">
							<div class="form-group">
								...
								<input class="form-control input-sm" name="input_product_unit_rate" id="input_product_unit_rate" placeholder="Unit Rate" autocomplete="off" />
							</div>
						</div>
						<div class="col-md-2" style="padding: 3px;">
							<div class="form-group">
						...
						<div class="col-md-2" style="padding: 3px;">
							<div class="form-group">
								<label>Disc %</label>
								<input class="form-control number input-sm" name="input_disc" id="input_disc" placeholder="Discount %" type="text" value=0 autocomplete="off"  />
							</div>
						</div>
						<div class="col-md-2" style="padding: 3px;">
							<div class="form-group">
	...
	<div class="box box-info">
		<div class="box-header with-border">
			...
			<div class="pull-right box-tools">
				<button type="button" class="btn btn-info btn-flat btn-sm" onclick="load_form_div('/Medical/Invoice_old/<?=$invoiceMaster[0]->id ?>','search_body_part');">Search OLD</button>
			</div>
		</div>
		<!-- /.box-header -->
		<!-- form start -->
...
<script>
	...
	function add_item_invoice()
	{
		...
		var HSNCODE=$("#input_HSNCODE").val();

		var mrp=$("#input_MRP").val();
		var packing=$("#input_Packing").val();

		var stock_empty=$("#input_stock_empty").val();
		...
		if(input_qty<=0)
		{
			...
			notify('error','Please Attention','Qty Should be Greater the 0');
			return false;
		}

		if(product_unit_rate<=0)
		{
			notify('error','Please Attention','Price Should Be Greater the 0');
			return false;
		}

		if(mrp<=0)
		{
			notify('error','Please Attention','MRP Should Be Greater the 0');
			return false;
		}		
		...
		if(item_code > 0)
		{
			if(input_qty > 0)
			{
				...
				if(parseInt(input_qty)<=parseInt(hid_c_qty) || hid_c_qty=='' )
				{
					$("#wait_for_next").val('1');
				}else{
			...
			{ 		
				...
				"disc": $('#input_disc').val(),
				"Batch_no": $('#input_Batch_no').val(),
				"ExpDt": $('#input_ExpDt').val(),
				"inv_id": inv_id,
				"item_code":item_code,
				"CGST":CGST,
				"SGST":SGST,
   ...
   $(document).ready(function(){
	   var cache = {};
		...
		select: function( event, ui ) {
			...
			$("#input_ExpDt").val(ui.item.l_Expiry);
			$("#input_batch").html(' | Batch No.:'+ui.item.l_Batch+' | Exp.Dt: '+ui.item.l_Expiry);
			$("#div_product_mrp").html(' | MRP:'+ui.item.l_mrp + ' | Unit Rate :' + ui.item.l_unit_rate);
			$("#input_product_unit_rate").val(ui.item.l_unit_rate);
			$("#item_code").val(ui.item.l_item_code);
			$("#hid_c_qty").val(ui.item.l_c_qty);
			$("#stock_product_qty").html(' |Qty : '+ui.item.l_c_qty+' |Pak :'+ui.item.l_packing);
			$("#input_MRP").val(ui.item.l_mrp);
			$("#input_Packing").val(ui.item.l_packing);

			$("#input_CGST").val(ui.item.l_CGST_per);
			$("#input_SGST").val(ui.item.l_SGST_per);
			$("#input_HSNCODE").val(ui.item.l_HSNCODE);
			
			if(ui.item.l_item_found==0){
				$('#collapse_zero_stock').collapse('show');
			}else{
				$('#collapse_zero_stock').collapse('hide');
			}
