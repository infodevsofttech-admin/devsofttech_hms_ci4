...
class PathLab extends MY_Controller {
	...
	public function confirm_payment()
	{
		...
		$sql="select sum(if(credit_debit=0,amount,amount*-1)) as tPayment from payment_history where payof_type=2 and payof_id='".$invoice_id."'";
		$query = $this->db->query($sql);
		...
		$showcontent1='<div class="row no-print">
						<div class="col-xs-6">
							<a href="/index.php/PathLab/invoice_print/'.$invoice_id.'" target="_blank" class="btn btn-default"><i class="fa fa-print"></i> Print</a>
						</div>
						<div class="col-xs-6">
							Payment Method by : ';          
	...
	public function confirm_payment1()
	{
		...
		if($amountpaid < $inv_master[0]->net_amount );
		{
			$amountbalanace=$inv_master[0]->net_amount-$amountpaid;
		}
	...
	public function IPD_Invoice_Edit($Invoice_ID)
	{
		...
		$sql="select *,if(ipd_status=0,DATEDIFF(sysdate(),register_date),DATEDIFF(discharge_date,register_date)) as no_days from  ipd_master  where  id=".$data['invoiceMaster'][0]->ipd_id;
        $query = $this->db->query($sql);
	...
	public function showinvoice($invoice_id)
	{
		...
		$sql="select * from invoice_item i join hc_item_type t on i.item_type=t.itype_id where t.is_ipd_opd=2 and i.inv_master_id=".$invoice_id;
		$query = $this->db->query($sql);
