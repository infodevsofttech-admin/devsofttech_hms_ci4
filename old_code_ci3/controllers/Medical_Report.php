...
class Medical_Report extends MY_Controller
{
	...
	//
	//Daily Medicine Doctor Wise Sale Report
	public function Report_daily_med_sale_doc()
	{
		...
		$this->load->view('Medical/Report/Med_Report_daily_med_doc_sale', $data);
	}

	public function Report_daily_med_sale_doc_data($sale_date_range, $doc_id, $output = 0)
	{
		$sql_where = " 1=1 ";
		...
		if ($doc_id <> 0) {
			...
			$sql_where .= " and m.doc_id  in (" . $doc_id_list . ")";
		}
		...
		$sql = "SELECT m.doc_id,d.p_fname, i.item_code,i.item_Name,
	...
	public function Report_company_med_purchase_data($sale_date_range, $comp_id, $output = 0)
	{
		...
		if (!$this->ion_auth->in_group('MedOLDRec')) {
			$sql_where .= " and m.date_of_invoice  >=date_add(sysdate(),interval -1 day)";
		}
	...
	public function Report_2_data($sale_date_range, $bill_type, $output = 0)
	{
		...
		if (!$this->ion_auth->in_group('MedOLDRec')) {
			$sql_where .= " and date(m.inv_date)  >=date(date_add(sysdate(),interval -2 day))";
		}
	...
	public function Report_6_data($sale_date_range, $output = 0)
	{
		...
		$sql = "SELECT p.ipd_code,p.p_code,p.p_fname, DATE_FORMAT(p.register_date,'%d-%m-%Y') as ipd_admit_date_str, DATE_FORMAT(p.discharge_date,'%d-%m-%Y') as ipd_discharge_date_str, p.admit_type, 
				(if(m.ipd_credit=1 and m.ipd_credit_type=1,g.net_amount,0)) as IPD_Credit_Amount, 
				(if(m.ipd_credit=1 and m.ipd_credit_type=0,g.net_amount,0)) as IPD_Package_Amount, 
				(if(m.ipd_credit=0,g.net_amount,0)) as IPD_CASH_Amount, (g.net_amount-pay_his.Total_Paid_Amount) as cash_balance,
				pay_his.Total_Paid_Amount as Paid_Amount 
				FROM ((invoice_med_master m join v_ipd_list p on m.ipd_id=p.id ) join inv_med_group g ON m.med_group_id=g.med_group_id) 
				LEFT JOIN (SELECT SUM(if(h.credit_debit=0,h.amount,h.amount*-1)) AS Total_Paid_Amount, h.ipd_id 
				FROM payment_history_medical h WHERE h.Customerof_type=2 GROUP BY h.ipd_id) AS pay_his ON p.id=pay_his.ipd_id
		...
		foreach ($stock_list as $row) {
			...
			$content .=  '	<td align="right">' . Round($row->cash_balance,0) . '</td>';
	...
	public function Report_Payment_Recieved_data($sale_date_range, $emp_name_id = 0, $output = 0)
	{
		...
		if (!$this->ion_auth->in_group('MedOLDRec')) {
			$sql_where .= " and p.payment_date  >=date_add(sysdate(),interval -1 day)";
		}
	...
	public function Report_3_purchase_data($sale_date_range, $output = 0)
	{
		$sql_where = " 1=1 ";
	...
	public function purchase_invoice_data($sale_date_range, $supplier_id, $output = 0)
	{
		...
		$sql = "select p.id,p.Invoice_no,p.date_of_invoice,p.sid,s.name_supplier,s.short_name,s.gst_no,
			...
			where " . $sql_where . " and s.state='" . M_State . "' group by p.id 	order by s.state,p.id";

		$query = $this->db->query($sql);
		$pur_inv_total_local = $query->result();
		...
		$query = $this->db->query($sql);
		$pur_inv_total_IGST = $query->result();


		$xcontent_head = '<table border="1" width="100%" cellpadding="3">';

		$xcontent_head .= '<thead><tr>
							<th width="50px">#</th>
							<th>Invoice ID</th>
							<th>Supplier</th>
							<th>GST</th>
							...
							<th>R Amount</th>
							<th>GST</th>	
							<th align="right">TotGST_per_0</th>
							<th align="right">CGST_per_0</th>
							<th align="right">SGST_per_0</th>
							<th align="right">TotGST_per_5</th>
							<th align="right">CGST_per_2_5</th>
							<th align="right">SGST_per_2_5</th>
							<th align="right">TotGST_per_12</th>
							<th align="right">CGST_per_6</th>
							<th align="right">SGST_per_6</th>
							<th align="right">TotGST_per_18</th>
							<th align="right">CGST_per_9</th>
							<th align="right">SGST_per_9</th>
							<th align="right">TotGST_per_28</th>
							<th align="right">CGST_per_14</th>
							<th align="right">SGST_per_14</th>
						</tr></thead><tbody>';
		...
		foreach ($pur_inv_total_local as $row) {
			$sr_no = $sr_no + 1;
			...
			$xcontent .= '	<tr>
							...
							<td align="right">' . round($row->TotGST_per_0) . '</td>
		...
		$SGST_Purchase_List = $xcontent_head . $xcontent . $xcontent_total;
		...
		$xcontent_head = '<table border="1" width="100%" cellpadding="3">
							...
							<tr>
								...
								<th align="right">TotGST_per_0</th>
								<th align="right">CGST_per_0</th>
								<th align="right">IGST_per_0</th>
								<th align="right">TotGST_per_5</th>
								<th align="right">CGST_per_2_5</th>
								<th align="right">IGST_per_2_5</th>
								<th align="right">TotGST_per_12</th>
								<th align="right">CGST_per_6</th>
								<th align="right">IGST_per_6</th>
								<th align="right">TotGST_per_18</th>
								<th align="right">CGST_per_9</th>
								<th align="right">IGST_per_9</th>
								<th align="right">TotGST_per_28</th>
								<th align="right">CGST_per_14</th>
								<th align="right">IGST_per_14</th>
							</tr></thead><tbody>';
		$sr_no = 0;
		$tamt = 0;
		$tramt = 0;
		...
		foreach ($pur_inv_total_IGST as $row) {
			$sr_no = $sr_no + 1;
			$tamt = $tamt + $row->tamount;
			$tramt = $tramt + round($row->tamount);
			$tgst = $tgst + round($row->gst);
		...
		$IGST_Purchase_List = $xcontent_head . $xcontent . $xcontent_total;

		$final_content = $SGST_Purchase_List;

		if (count($pur_inv_total_IGST) > 0) {
			$final_content .= "<table>
								<tr>
									<td></td>
								</tr>
							</table>";
			$final_content .= $IGST_Purchase_List;
		}

		//echo $content;
		if ($output == 0) {
			create_report_pdf_landscape($final_content, 'Med_Invoice');
		} else {
			ExportExcel($final_content, 'Report_Medical');
		}
	}
	...
	public function purchase_invoice_data_pdf($sale_date_range, $supplier_id, $output = 0)
	{
		...
		$sql = "select p.id,trim(s.name_supplier) AS name_supplier,p.sid,p.Invoice_no, 
				...
				p.date_of_invoice, s.short_name,s.gst_no,s.state,
		...
		foreach ($pur_inv_total as $row) {
			...
			if ($row->id != '' && $row->name_supplier != '' && $row->date_of_invoice != '') {
				if ($head_content != $row->name_supplier) {
					...
					$content .= '<tr>
								<td colspan="8">' . $row->name_supplier . ' / GST No. ' . $row->gst_no . ' / State : ' . $row->state . '   </td>
							</tr>';
					$content .= '<tr>
								<td colspan="2"></td>
								<td>#</td>
	...
	public function purchase_invoice_data_gsr_rate_pdf($sale_date_range, $supplier_id, $output = 0)
	{
		$sql_where = " 1=1 ";
		...
		$sql = "SELECT m.id,m.supp_name,m.date_of_invoice,m.Invoice_no,s.name_supplier,s.sid,
				s.short_name,s.gst_no,s.state,
				t.CGST_per,sum(t.qty) as t_qty,
				sum(t.CGST) as TCGST,
				t.SGST_per,
				sum(t.SGST) as TSGST,
				sum(t.CGST+t.SGST) as TGST,
				SUM(t.taxable_amount) as TaxableAmount,
				SUM(t.net_amount) as Amount
				FROM (purchase_invoice m join purchase_invoice_item t
				ON m.id=t.purchase_id)
				JOIN med_supplier s ON m.sid=s.sid
			where " . $sql_where . " group BY s.sid,m.id,t.CGST_per with ROLLUP";
	...
	public function Report_5_data($sale_date_range, $output = 0)
	{
		...
		$sql = "select m.inv_med_code,Date_Format(m.inv_date,'%d-%m-%Y') as Inv_Date,m.inv_name,
				...
				sum(i.TaxableAmount) as Total_TaxableAmount
		...
		foreach ($stock_list as $row) {
			...
			$content .=  '	<td align="right">' . $row->No_of_Qty . '</td>';
	...
	public function Report_5_HSNdata($sale_date_range, $output = 0)
	{
		...
		$sql = "select Left(i.HSNCODE," . M_HSNCODE_LEN . ") as HSNCODE,
				count(i.id) as No_of_Items,
				sum(i.qty) as No_of_Qty,
				i.CGST_per ,
				sum(i.twdisc_amount) as Total_Amount,
				...
				left join patient_master p on m.patient_id=p.id)
				left join organization_case_master o on m.case_id=o.id
				where " . $where . " group by Left(i.HSNCODE," . M_HSNCODE_LEN . ") ,i.CGST_per 	";
		...
		$content = '<table border="1" cellspacing=0 width="100%"  >
						<tr>
							...
							<th align="center">Item Qty</th>
							<th align="center" >Tax Per.</th>
							<th align="center">Tot.Amt.</th>
							<th align="center">Tot.Tax.Amt</th>
							<th align="center">Tax.Amt.5%</th>
							<th align="center">CGST 2.5%</th>
		...
		foreach ($stock_list as $row) {
			$content .=  '<tr>';
			$content .=  '	<td align="left">' . $row->HSNCODE . '</td>';
			$content .=  '	<td align="left">' . $row->No_of_Qty . '</td>';
			$content .=  '	<td align="left">' . $row->CGST_per . '</td>';
			$content .=  '	<td align="right">' . $row->Total_Amount . '</td>';
			$content .=  '	<td align="right">' . $row->Total_TaxableAmount . '</td>';
			$content .=  '	<td align="right">' . $row->Sale_5_Amount . '</td>';
			$content .=  '	<td align="right">' . $row->CGST_2_5 . '</td>';
		...
		ExportExcel($content, 'Report_Medical_gst_hsn');
	}
	...
	public function Short_medicine()
	{
		...
		$this->load->view('Medical/Stock/Short_medicine', $data);
	}

	public function Short_medicine_data($sale_date_range, $formulation, $output = 0)
	{
		$rangeArray = explode("S", $sale_date_range);
		$minRange = $rangeArray[0];
		$maxRange = $rangeArray[1];

		$formulation_list = str_replace("S", ",", $formulation);

		$content = "";

		$where = "  s.short_date between '" . $minRange . "' and '" . $maxRange . "'";

		if ($formulation_list <> '0') {
			$where .= " and m.formulation_id in (" . $formulation_list . ")";
		}

		$sql = "SELECT s.id,s.item_Name,s.curQty,m.formulation,
				date_format(s.short_date,'%d-%m-%Y') as str_short_date,
				s.supplier_name
				FROM med_short_list s join med_product_master m on s.item_code=m.id
				where " . $where . " order by short_date desc 	";
		...
		foreach ($stock_list as $row) {
			$content .=  '<tr>';
			$content .=  '	<td align="left">' . $row->id . '</td>';
			$content .=  '	<td align="left">' . $row->str_short_date . '</td>';
			$content .=  '	<td align="left">' . $row->item_Name . '</td>';
			$content .=  '	<td align="left">' . $row->formulation . '</td>';
			$content .=  '	<td align="right">' . Round($row->curQty) . '</td>';
			$content .=  '	<td align="left">' . $row->supplier_name . '</td>';
		...
		if ($output == 0) {
			...
			$file_name = 'Report_Medicine_Short_' . date('Ymdhis') . ".pdf";
	...
	{
		$this->load->view('Medical/Report/Report_gst_1');
	}
}
