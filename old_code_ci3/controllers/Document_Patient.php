...
class Document_Patient extends MY_Controller
{
	...
	{
		$sql = "select *,IFNULL(GET_AGE_BY_DOB(dob),age) as str_age,Date_Format(insert_date,'%Y-%m-%d') as str_regdate,if(gender=1,'Male','Female') as xgender from patient_master where  id=" . $pno;
