...
class Product_stock_master extends MY_Controller {
	...
	{
		$this->load->view('Medical/Storestock/drug_master_list');
	}
	...
	public function Product_search()
	{
		...
		$this->load->view('Medical/Storestock/product_search_result',$data);
	}
	...
	public function Product_edit($Product_id)
	{
		...
		$this->load->view('Medical/Storestock/product_edit',$data);
	}

	public function Product_delete($Product_id)
	{
		$sql="Select count(*) as no_rec from inv_stock_item where item_code=".$Product_id;
		$query = $this->db->query($sql);
		$product_sale_data= $query->result();

		$sql="Select count(*) as no_rec from purchase_invoice_item_stock where item_code=".$Product_id;
		$query = $this->db->query($sql);
		...
		if($product_sale_data[0]->no_rec>0 || $product_purchase_data[0]->no_rec>0)
		{
			...
			$this->load->model('Product_stock_master_M');
			$is_delete_master=$this->Product_stock_master_M->delete_master($Product_id);
	...
	public function NewProduct()
	{
		...
		$this->load->view('Medical/Storestock/product_new',$data);
	}
	...
	public function product_master_update($product_id=0)
	{
		...
		if($product_id>0)
		{
		 	...
		 	$original_value = $this->db->query("SELECT item_name FROM med_store_product_master WHERE id = ".$product_id)->row()->item_name ;
         ...
         if ($this->form_validation->run() == TRUE)
            {
				...
				if($product_id<1)
				{
					...
					$is_update_stock=$this->Product_stock_master_M->insert($Pdata);
				}else{
					$Pdata['update_by'] = $user_name_info;
					$is_update_stock=$this->Product_stock_master_M->update($Pdata,$product_id);
				}
