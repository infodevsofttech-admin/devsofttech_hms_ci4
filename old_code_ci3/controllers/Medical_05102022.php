...
class Medical extends MY_Controller {
	...
	public function getInvoiceTable()
	{
		...
		if( !empty($requestData['columns'][3]['search']['value']) ){  //patient_code
			$stext=$requestData['columns'][4]['search']['value'];
	...
	{
		$sql = "SELECT i.*,p.p_code,p.p_fname,p.mphone1,m.payment_received,m.payment_balance
		FROM (inv_med_group m JOIN ipd_master i ON m.ipd_id=i.id)
		JOIN patient_master p ON i.p_id=p.id
		WHERE i.ipd_status=0 AND m.med_type=1  AND m.net_amount>0
		AND payment_balance>=50000 ";
	...
	public function Invoice_Item_old()
	{
		...
		if($search_type==3){
			...
			i.*,date_format(i.expiry,'%m-%Y') as exp_date,(i.disc_amount+disc_whole) as t_discount
			FROM (invoice_med_master m JOIN inv_med_item i ON m.id=i.inv_med_id AND m.sale_return=0)
			Where m.inv_date >=date_add(curdate(),interval -5 month) and m.inv_med_code like '".$search_text."%' and m.ipd_id=0 and m.case_id=0";
			$query = $this->db->query($sql);
			$data['med_inv_item_list']= $query->result();
		}
