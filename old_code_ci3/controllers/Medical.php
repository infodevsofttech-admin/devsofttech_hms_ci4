...
class Medical extends MY_Controller
{
	...
	public function purchase_scan_doc($purchase_inv_id)
	{
		$data['purchase_inv_id'] = $purchase_inv_id;

		$sql = "select * from purchase_invoice where id='" . $purchase_inv_id . "' ";
		$query = $this->db->query($sql);
		$data['purchase_invoice'] = $query->result();

		$sql = "select * from purchase_invoice_scan_file where purchase_id='" . $purchase_inv_id . "' ";
		$query = $this->db->query($sql);
		$data['purchase_invoice_scan_file'] = $query->result();

		$this->load->view('Medical/Purchase/purchase_invoice_scan', $data);
	}

	public function purchase_file_list($inv_id)
	{
		$sql = "select * from purchase_invoice_scan_file where purchase_id=$inv_id";
		$query = $this->db->query($sql);
		$data['purchase_file_rec'] = $query->result();

		$this->load->view('Medical/Purchase/purchase_file_show', $data);
	}

	public function save_image($purchase_id)
	{
		$filename =  'purchase-' . $purchase_id . '-' . time() . '.jpg';

		$folder_name = 'medical_uploads/purchase_invoice/' . date('Ymd');
		...
		if (!$this->upload->do_upload('webcam')) {
			...
			$this->load->model('FileMedical_M');

			$file_insert_id = $this->FileMedical_M->insert_purchase_scan($data['upload_data']);
			...
			if ($file_insert_id > 0) {
				$this->FileMedical_M->update_purchase_scan($udata, $file_insert_id);
			}
	...
	public function purchase_file_last_list($purchase_id = '')
	{
		
		$sql = "select * from purchase_invoice_scan_file where purchase_id=$purchase_id order by id desc limit 5";
		$query = $this->db->query($sql);
		$data['purchase_invoice_scan_file'] = $query->result();

		$this->load->view('Medical/Purchase/scan_purchase_list', $data);
	}

	public function purchase_file_hide($file_id, $purchase_id = '')
	{
		$user = $this->ion_auth->user()->row();
		...
		$this->FileMedical_M->delete_purchase_scan_fileupload( $file_id);

		redirect('/Medical/purchase_file_last_list/' . $purchase_id);
	}


	public function PurchaseNew()
	{
		$sql = "select * from med_supplier where active=1 order by name_supplier";
		$query = $this->db->query($sql);
		$data['supplier_data'] = $query->result();
	...
	public function challan_invoice($supplier_id)
	{
		...
		FROM (purchase_invoice p join med_supplier s on p.sid=s.sid)
		JOIN purchase_invoice_item t ON p.id=t.purchase_id
		...
		$this->load->view('Medical/Purchase/purchase_challan_list', $data);
	}

	public function challan_item_to_purchase($ss_no, $purchase_id)
	{
		$sql = "select * from purchase_invoice_item  where id=$ss_no";
		$query = $this->db->query($sql);
		...
		if (count($purchase_invoice_item) > 0) {
			...
			$this->load->model('Medical_M');
			$is_update_stock = $this->Medical_M->update_purchase_item($Pdata, $ss_no);
	...
	{
		$sql = "select * from purchase_invoice_item  where old_purchase_id>0 and id=$ss_no";
		$query = $this->db->query($sql);
		$purchase_invoice_item = $query->result();
		...
		if (count($purchase_invoice_item) > 0) {
			...
			$sql = "select * from purchase_invoice  where id=$old_challan_id";
			$query = $this->db->query($sql);
	...
	public function SupplierUpdate($sid = 0)
	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			$active = ($this->input->post('chk_active') == 'on' ? 1 : 0);
	...
	public function purchase_invoice_item_list_old($item_code)
	{
		...
		i.Item_name,i.packing,i.purchase_price,i.qty,i.qty_free,i.mrp,i.discount,i.sch_disc_per,i.purchase_unit_rate,p.Invoice_no
	...
	public function purchase_invoice_item_delete($inv_id, $inv_item_del)
	{
		...
		$sql = "select * from purchase_invoice_item where id='$inv_item_del' ";
		$query = $this->db->query($sql);
		$drug_item = $query->result();
	...
	public function Invoice_med_show($inv_id)
	{
		...
		FROM (inv_med_item i JOIN purchase_invoice_item p ON i.item_code=p.item_code AND p.remove_item=0  )
		where inv_med_id=" . $inv_id . " GROUP BY i.id";
		$query = $this->db->query($sql);
		$data['inv_items'] = $query->result();

		$sql = "select item_code
		from inv_med_item   where inv_med_id=" . $inv_id . " Group by item_code Having count(id)>1";
		$query = $this->db->query($sql);
	...
	public function Invoice_med_datasheet($inv_id)
	{
		...
		$sql = "select *,ifnull(DATEDIFF(expiry,CURDATE()),1000) AS no_day from inv_med_item where inv_med_id=" . $inv_id;
		$query = $this->db->query($sql);
		$data['inv_items'] = $query->result();
	...
	public function check_invoice($pno, $ipd_id, $case_id)
	{
		$sql = "select *
		 from invoice_med_master where inv_date=curdate() and patient_id='" . $pno . "' ";
		...
		$sql = "SELECT m.id AS inv_id,m.inv_med_code,m.inv_date,m.inv_name,m.inv_phone_number,
		 ...
		 Where  m.patient_id=$pno and  m.inv_date=curdate()
		 order by m.id desc";
		...
		if (count($data['med_inv_item_list']) > 0) {

			$this->load->view('Medical/mini_invoice', $data);
		} else {
			redirect('Medical/Invoice_counter_new/' . $pno . '/' . $ipd_id . '/' . $case_id);
		}
	}
	...
	public function Invoice_counter_new($pno, $ipd_id, $case_id)
	{
		...
		if ($inser_id) {
			...
			SUM(p.total_unit-p.total_sale_unit-total_return_unit-total_lost_unit)/p.packing AS cur_qty
			FROM (inv_med_item i JOIN purchase_invoice_item p ON i.item_code=p.item_code AND p.remove_item=0 )
			where inv_med_id=" . $inser_id . " GROUP BY i.id";
			$query = $this->db->query($sql);
			$data['inv_items'] = $query->result();

			$sql = "select item_code
			from inv_med_item   where inv_med_id=" . $inser_id . " Group by item_code Having count(id)>1";
			$query = $this->db->query($sql);
			$data['inv_items_multiple'] = $query->result();
	...
	public function get_drug()
	{
		if (isset($_GET['term'])) {
			...
			$sql = "SELECT p.item_name as item_name,p.formulation,t.batch_no,t.expiry_date as expiry_date_str,t.expiry_date,
				DATEDIFF(LAST_DAY(t.expiry_date),curdate()) as expiry_month_pending,
				(t.total_unit-t.total_sale_unit-total_lost_unit-t.total_return_unit) AS c_qty,
				t.id,t.mrp,t.selling_unit_rate,p.id AS item_code,t.packing,if(t.stock_date>'2025-09-21',1,0) as new_stock
				FROM med_product_master p 
				JOIN purchase_invoice_item t ON p.id=t.item_code 
				and (t.total_unit-t.total_sale_unit-t.total_lost_unit-t.total_return_unit)>0
				and t.remove_item=0 and t.item_return=0
				and (LAST_DAY(expiry_date)>date_add(sysdate(),interval -15 day) or p.exp_date_applicable=0)
				where	(p.item_name like '" . $q . "%' ";

			if (strlen($q) > 5) {
				$sql = $sql . " or p.item_name SOUNDS LIKE '" . $q . "' or p.genericname like '%" . $q . "%'";
			}

			$sql = $sql . ") order by p.item_name,t.id limit 100";
			...
			if ($query->num_rows() > 0) {
				foreach ($query->result_array() as $row) {
					$msg_desc = '';
					$no_exp_month = Round($row['expiry_month_pending'] / 30, 2);

					if ($no_exp_month <= 4 && $no_exp_month >= 2) {
						$msg_desc .= ' <span  style="color: blue;">Expire Within : ' . htmlentities(stripslashes($no_exp_month)) . ' Month </span>';
					} elseif ($no_exp_month == 1) {
						$msg_desc .= ' <span  style="color: Orange;">Expired : ' . htmlentities(stripslashes($no_exp_month)) . ' Month  </span>';
					} elseif ($no_exp_month < 1) {
						$msg_desc .= ' <span  style="color: red;">Expired : ' . htmlentities(stripslashes($no_exp_month)) . ' Month  </span>';
					}
					...
					$new_row['expiry_alert'] = htmlentities(stripslashes($no_exp_month));
					$new_row['desc'] = $msg_desc;
					...
					$new_row['l_new_stock'] = htmlentities(stripslashes($row['new_stock']));
					$new_row['sql'] = $sql;
	...
	public function getInvoiceTable()
	{
		...
		m.inv_phone_number, m.patient_code,
		if(m.ipd_id>0,Concat('<a href=\'javascript:load_form_div(\"/Medical/list_med_inv/',i.id,'\",\"maindiv\")\'>',m.ipd_code,'</a><br>'),'')  AS IPD_CODE,
		if(m.case_id>0,Concat(o.case_id_code,'<br>'),'') as ORG_CODE ,
		m.inv_date,Date_Format(m.inv_date,'%d-%m-%Y') as inv_date_str,
		m.net_amount,m.id,m.payment_received,m.payment_balance ";
		...
		left join inv_med_group g on i.id=g.ipd_id AND g.med_group_id=m.med_group_id";

		//$total_sql=$sql_count.$sql_from;
		$total_sql = "Select count(*) as no_rec from invoice_med_master";
		...
		if (!empty($requestData['columns'][2]['search']['value'])) {  //patient_code
			...
			$sql_where .= " AND (patient_code LIKE '%" . $stext . "' )";
			$sql_where_flag = 1;
		}

		if (!empty($requestData['columns'][3]['search']['value'])) {  //patient_code
			$stext = $requestData['columns'][3]['search']['value'];
			$sql_where .= " AND (m.ipd_code LIKE '%" . $stext . "' AND m.ipd_id>0 ) ";
			$sql_where_flag = 1;
		}
	...
	public function Adjust_Payment()
	{
		if (!$this->input->is_ajax_request()) {
			exit('no valid req.');
		}
		...
		if (count($inv_master) > 0) {
			if ($inv_master[0]->ipd_id == 0) {

				$discount = $inv_master[0]->discount_amount;
				$discount = $discount + $input_amount_paid;

				$log = $inv_master[0]->log;

				$update_str = "Adjust Balance Amt:" . $input_amount_paid;

				$log = $log . PHP_EOL . PHP_EOL . $update_str;
	...
	public function final_invoice($inv_id)
	{
		...
		//$this->Medical_M->update_invoice_group($inv_id);
		//$this->Medical_M->opd_reset( $inv_id);
		$this->Medical_M->update_invoice_final($inv_id);

		$sql = "select *,(discount_amount+item_discount_amount) as inv_disc_total,(CGST_Tamount+SGST_Tamount) as TGST ,date_add(ifnull(confirm_invoice,inv_insert_datetime) ,interval 240 minute) as bill_open_date,
			if( date_add(ifnull(confirm_invoice,inv_insert_datetime) ,interval 48 hour)>sysdate(),0,1) as bill_opd_close
		...
		$sql = "select s.id,s.pay_type,m.bank_name
				From medical_bank m join medical_bank_payment_source s on m.id=s.bank_id";
		$query = $this->db->query($sql);
        ...
        where  Medical_invoice_id=$inv_id";
		$query = $this->db->query($sql);
		$data['payment_history'] = $query->result();
	...
	public function add_item($type, $req = 0)
	{
		...
		if ($type == 1) {
			...
			$this->Medical_M->delete_invoiceitem($itemid, $inv_items);
		}
	...
	public function list_org_ipd()
	{
		...
		$sql = "SELECT CONCAT('IPD : ',i.ipd_code,' /Name :',p.p_fname,' / Admit Dt: ',date_format(i.register_date,'%d-%m-%Y'),
		' /Balance :',m.payment_balance) AS show_content
		FROM (inv_med_group m JOIN ipd_master i ON m.ipd_id=i.id)
		JOIN patient_master p ON i.p_id=p.id
		WHERE i.ipd_status=0 AND m.med_type=1  AND m.net_amount>0
		AND payment_balance>=50000 ";
		$query = $this->db->query($sql);
		$data['high_balance'] = $query->result();

		$this->load->view('Medical/Medical_ipd_org_list', $data);
	}

	public function list_high_balance()
	{
		$sql = "SELECT i.ipd_code,p.p_code,p.p_fname,p.mphone1,m.payment_received,m.payment_balance,m.net_amount
		FROM (inv_med_group m JOIN ipd_master i ON m.ipd_id=i.id)
		JOIN patient_master p ON i.p_id=p.id
		WHERE i.ipd_status=0 AND m.med_type=1  AND m.net_amount>0
		AND payment_balance>=50000 ";
		...
		$this->load->view('Medical/Report/Med_high_pending', $data);
	}
	...
	public function list_med_inv($ipd_id)
	{
		...
		if (count($data['med_group_list']) > 0) {
			if ($data['med_group_list'][0]->discount_group_2 <> 0) {
				exit('Please Contact to Medical Store Admin ');
			}
		}
	...
	public function payment_receipt($ipd_id, $cash = 0)
	{
		...
		$sql = "select m.p_id,p.p_fname,m.ipd_code,p.p_code, m.doc_name,m.mphone1,m.P_mobile1,m.P_mobile2,m.add1,m.add2,
	...
	public function Invoice_Item_Return_new($inv_type_id, $inv_type = 0)
	{
		$data['inv_type_id'] = $inv_type_id;
		$data['inv_type'] = $inv_type;
		...
		$this->load->view('Medical/IPD_return_medicine', $data);
	}

	public function IPD_Item_Return_temp($inv_type_id, $inv_type = 0)
	{
		$data['inv_type_id'] = $inv_type_id;
		$data['inv_type'] = $inv_type;
		...
		if ($inv_type == 0) {
			...
			join inv_med_item_return r on t.id=r.inv_item_id and return_status=0
			where m.ipd_id=" . $inv_type_id . "  order by r.id desc";
		} elseif ($inv_type == 1) {
			...
			$sql_items = "select m.id as inv_id,m.inv_med_code,date_format(inv_date,'%d-%m-%Y') as str_inv_date,m.ipd_id,m.patient_id,m.group_invoice_id,m.case_id,
				...
				from (invoice_med_master m join inv_med_item t on m.id=t.inv_med_id ) 
				join inv_med_item_return r on t.id=r.inv_item_id and return_status=0
				where m.case_id=" . $inv_type_id . "   order by r.id desc";
		} else {
			...
			$sql_items = "select m.id as inv_id,m.inv_med_code,date_format(inv_date,'%d-%m-%Y') as str_inv_date,m.ipd_id,m.patient_id,m.group_invoice_id,m.case_id,
				...
				from (invoice_med_master m join inv_med_item t on m.id=t.inv_med_id ) 
				join inv_med_item_return r on t.id=r.inv_item_id and return_status=0
				where l m.id=" . $inv_type_id . "   order by r.id desc";
		}

		$query = $this->db->query($sql_items);
		$data['inv_items'] = $query->result();

		$this->load->view('Medical/Medical_Invoice_Return_list', $data);
	}

	public function IPD_Item_list($inv_type_id)
	{
		$data['inv_type_id'] = $inv_type_id;
		...
		if (isset($_GET['term'])) {
			...
			$sql = "select m.id as inv_id,m.inv_med_code,date_format(inv_date,'%d-%m-%Y') as str_inv_date,m.ipd_id,m.patient_id,m.group_invoice_id,m.case_id,
			t.id as item_id,t.item_Name,t.qty,t.batch_no,t.expiry,t.store_stock_id as ss_no,t.item_code,t.mrp,t.price,
			t.amount,t.disc_amount,t.tamount,r.r_qty,r.id as return_id
			from (invoice_med_master m join inv_med_item t on m.id=t.inv_med_id ) 
			left join inv_med_item_return r on t.id=r.inv_item_id and return_status=0
			where r.id is null and m.ipd_id=" . $inv_type_id . " ";

			if (strlen($q) > 0) {
				$sql .= " and t.item_name LIKE '" . $q . "%'";
			} else {
				$sql .= " and 1<>1 ";
			}

			$sql .= "  order by t.item_name,t.id";

			$query = $this->db->query($sql);

			if ($query->num_rows() > 0) {
				foreach ($query->result_array() as $row) {
					$new_row['label'] = htmlentities(stripslashes($row['item_Name'])) . ' |B:' . htmlentities(stripslashes($row['batch_no'])) . ' |Rs.' . htmlentities(stripslashes($row['mrp'])) . ' |Qty:' . htmlentities(stripslashes($row['qty']));
					$new_row['value'] = htmlentities(stripslashes($row['item_Name']));
					$new_row['l_item_code'] = htmlentities(stripslashes($row['item_id']));
					$new_row['l_Batch'] = htmlentities(stripslashes($row['batch_no']));
					$new_row['l_mrp'] = htmlentities(stripslashes($row['mrp']));
					$new_row['l_price'] = htmlentities(stripslashes($row['price']));
					$new_row['l_qty'] = htmlentities(stripslashes($row['qty']));
					$new_row['sql'] = $sql;
					$row_set[] = $new_row; //build an array
				}
				echo json_encode($row_set); //format the array into json data
	...
	public function Invoice_Item_old()
	{
		...
		if ($search_type == 1) {
			...
			i.*,date_format(i.expiry,'%m-%Y') as exp_date,(i.disc_amount+disc_whole) as t_discount
			FROM (invoice_med_master m JOIN inv_med_item i ON m.id=i.inv_med_id AND m.sale_return=0)
			Where m.inv_date >=date_add(curdate(),interval -5 month) and  m.inv_phone_number='$search_text'  and m.ipd_id=0 and m.case_id=0 ";
			$query = $this->db->query($sql);
			$data['med_inv_item_list'] = $query->result();
		}
		...
		if ($search_type == 2) {
			...
			i.*,date_format(i.expiry,'%m-%Y') as exp_date,(i.disc_amount+disc_whole) as t_discount
			FROM (invoice_med_master m JOIN inv_med_item i ON m.id=i.inv_med_id AND m.sale_return=0)
			Where m.inv_date >=date_add(curdate(),interval -5 month) and m.patient_id in ($search_id_list) and m.ipd_id=0 and m.case_id=0";
			$query = $this->db->query($sql);
			$data['med_inv_item_list'] = $query->result();
		}
		...
		if ($search_type == 3) {
			...
			i.*,date_format(i.expiry,'%m-%Y') as exp_date,(i.disc_amount+disc_whole) as t_discount
			FROM (invoice_med_master m JOIN inv_med_item i ON m.id=i.inv_med_id AND m.sale_return=0)
			Where m.inv_date >=date_add(curdate(),interval -12 month) and m.inv_med_code like '%" . $search_text . "' and m.ipd_id=0 and m.case_id=0";
			$query = $this->db->query($sql);
			$data['med_inv_item_list'] = $query->result();
		}
		...
		if ($search_type == 4) {
			...
			i.*,date_format(i.expiry,'%m-%Y') as exp_date,(i.disc_amount+disc_whole) as t_discount
			FROM (invoice_med_master m JOIN inv_med_item i ON m.id=i.inv_med_id AND m.sale_return=0)
			Where m.inv_date >=date_add(curdate(),interval -5 month) and i.item_Name like '" . $search_text . "%' ";
			$query = $this->db->query($sql);
			$data['med_inv_item_list'] = $query->result();
		}
	...
	public function Invoice_old($inv_id)
	{
		...
		if ($ipd_id > 0) {
			...
			m.net_amount
			FROM (invoice_med_master m JOIN inv_med_item i ON m.id=i.inv_med_id AND m.sale_return=0)
			Where  m.patient_id='$p_id' and m.id<>$inv_id and m.ipd_id=0 and m.case_id=0 
			and m.inv_date>date_add(sysdate(),interval -6 month)
			order by m.id desc";
		} else if ($phone_no != '') {
			$sql = "SELECT m.id AS inv_id,m.inv_med_code,m.inv_date,m.inv_name,m.inv_phone_number,
			i.*,date_format(i.expiry,'%m-%Y') as exp_date,(i.disc_amount+disc_whole) as t_discount,
			m.net_amount
			FROM (invoice_med_master m JOIN inv_med_item i ON m.id=i.inv_med_id AND m.sale_return=0)
			Where  m.inv_phone_number='$phone_no' and m.id<>$inv_id   and m.ipd_id=0 and m.case_id=0 
			and m.inv_date>date_add(sysdate(),interval -6 month)
			order by m.id desc";
		} else {
			$sql = "SELECT m.id AS inv_id,m.inv_med_code,m.inv_date,m.inv_name,m.inv_phone_number,
			...
			m.net_amount
			FROM (invoice_med_master m JOIN inv_med_item i ON m.id=i.inv_med_id AND m.sale_return=0)
			Where  1<>1 and m.inv_date>date_add(sysdate(),interval -6 month)";
		}
	...
	public function Med_Return_print($inv_type_id, $inv_type = 0)
	{
		...
		if ($inv_type == 0) {
			...
			$sql_items = "select m.id as inv_id,m.inv_med_code,date_format(inv_date,'%d-%m-%Y') as str_inv_date,
			date_format(return_date_time,'%d-%m-%Y') as str_ret_date,m.ipd_id,m.patient_id,m.group_invoice_id,m.case_id,
			t.id as ritem_id,t.inv_item_id,t.item_Name,t.qty,t.batch_no,t.expiry,t.store_stock_id as ss_no,t.item_code,t.mrp,t.price,
			t.update_remark,t.return_date_time,
			t.amount,t.disc_per,t.disc_amount,t.tamount,t.r_qty,t.id as return_id
			from (invoice_med_master m join inv_med_item_return t on m.id=t.inv_med_id and t.return_status=1)  
			where m.ipd_id=" . $inv_type_id . " order by t.return_date_time,t.item_Name,t.batch_no";
			...
			date_format(return_date_time,'%d-%m-%Y') as str_ret_date,m.ipd_id,m.patient_id,m.group_invoice_id,m.case_id,
				...
				t.amount,t.disc_per,t.disc_amount,t.tamount,t.r_qty,t.id as return_id
				from (invoice_med_master m join inv_med_item_return t on m.id=t.inv_med_id and t.return_status=1) 
				where m.case_id=" . $inv_type_id . "  order by t.return_date_time,t.item_Name,t.batch_no";
			...
			$sql_items = "select m.id as inv_id,m.inv_med_code,date_format(inv_date,'%d-%m-%Y') as str_inv_date,
				...
				t.amount,t.disc_per,t.disc_amount,t.tamount,t.r_qty,t.id as return_id
				from (invoice_med_master m join inv_med_item_return t on m.id=t.inv_med_id and t.return_status=1) 
				where m.id=" . $inv_type_id . "   order by t.return_date_time,t.item_Name,t.batch_no";
		...
		$file_name = 'Medical_Return_' . date('Ymdhis') . ".pdf";

		$filepath = $file_name;

		$content = $this->load->view('Medical/Print/Medical_return_medicine', $data, TRUE);
	...
	public function get_medical_bill_pending_higher()
	{
		$sql = "SELECT COUNT(i.id) AS No_IPD
		FROM inv_med_group m JOIN ipd_master i ON m.ipd_id=i.id
		WHERE i.ipd_status=0 AND m.med_type=1  AND m.net_amount>0
		AND payment_balance>=50000";
		$query = $this->db->query($sql);
		$ipd_pending_balance = $query->result();

		$No_IPD = $ipd_pending_balance[0]->No_IPD;
		if ($No_IPD > 0) {
			echo 'High Bal. :' . $No_IPD;
		} else {
			echo '';
		}
	}
