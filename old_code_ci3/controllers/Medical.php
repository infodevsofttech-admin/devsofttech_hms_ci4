...
class Medical extends MY_Controller {
	...
	public function get_drug(){
		if (isset($_GET['term'])){
			...
			$sql="SELECT p.item_name as item_name,p.formulation,t.batch_no,t.expiry_date as expiry_date_str,t.expiry_date,
				DATEDIFF(t.expiry_date,curdate()) as expiry_month_pending,
				(t.total_unit-t.total_sale_unit-total_lost_unit-t.total_return_unit) AS c_qty,
				...
				and (expiry_date>date_add(sysdate(),interval -1 month) or p.exp_date_applicable=0)
				where	(p.item_name like '".$q."%' or p.genericname like '".$q."%'";
			...
			if($query->num_rows() > 0){
			  foreach ($query->result_array() as $row){
				$msg_desc='';
				$no_exp_month=Round($row['expiry_month_pending']/30,0);
	...
	public function final_invoice($inv_id)
	{
		...
		$sql="select *,(discount_amount+item_discount_amount) as inv_disc_total,(CGST_Tamount+SGST_Tamount) as TGST ,date_add(inv_date,interval 24 hour) as bill_open_date
