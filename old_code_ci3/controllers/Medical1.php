...
class Medical extends MY_Controller {
	...
	public function Invoice_counter_new($pno,$ipd_id,$case_id)
    {
		...
		if($inser_id)
		{
			...
			$sql="select *,ifnull(DATEDIFF(expiry,CURDATE()),-1000) AS no_day from inv_med_item where inv_med_id=".$inser_id;
			$query = $this->db->query($sql);
			$data['inv_items']= $query->result();
	...
	public function get_drug(){
		if (isset($_GET['term'])){
			...
			$sql="SELECT p.item_name,t.batch_no,t.expiry_date as expiry_date_str,t.expiry_date,
				(t.total_unit-t.total_sale_unit-total_lost_unit) AS c_qty,
				t.id,t.mrp,t.selling_unit_rate,p.id AS item_code,p.packing
				FROM med_product_master p 
				JOIN purchase_invoice_item t ON p.id=t.item_code and (t.total_unit-t.total_sale_unit-t.total_lost_unit)>0
			...
			if($query->num_rows() > 0){
			  foreach ($query->result_array() as $row){
				$new_row['label']=htmlentities(stripslashes($row['item_name'])).' |B:'.htmlentities(stripslashes($row['batch_no'])). ' |Pak:'.htmlentities(stripslashes($row['packing'])).' |Rs.'.htmlentities(stripslashes($row['mrp'])).' |Qty:'.htmlentities(stripslashes($row['c_qty']));
	...
	public function add_item($type,$req=0)
	{
		...
		if($type==1)
		{
			...
			$sql="SELECT  p.item_name,t.batch_no,t.expiry_date,
					...
					p.HSNCODE,p.CGST_per,p.SGST_per,t.packing
					FROM med_product_master p 
					JOIN purchase_invoice_item t ON p.id=t.item_code
			...
			$sql="select *,ifnull(DATEDIFF(expiry,CURDATE()),-1000) AS no_day from inv_med_item where inv_med_id=".$inv_id."  and store_stock_id=".$store_stock_id;
			$query = $this->db->query($sql);
		...
		else if($type==2)
		{
			...
			$sql="select *,ifnull(DATEDIFF(expiry,CURDATE()),-1000) AS no_day from inv_med_item  where id=".$itemid;
			$query = $this->db->query($sql);
		...
		if($inser_id>0)
		{
			...
			$sql="select *,ifnull(DATEDIFF(expiry,CURDATE()),-1000) AS no_day
