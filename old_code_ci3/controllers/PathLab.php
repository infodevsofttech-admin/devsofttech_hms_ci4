...
class PathLab extends MY_Controller {
	...
	public function confirm_payment()
	{
		...
		$bank_id=$this->input->post('cbo_pay_type');
			...
			if ($this->form_validation->run() == TRUE)
            {
				...
				if($amountpaid<=$amountbalanace)
				{
					...
					$data = array( 
						...
						'payment_status'=>'1',
						'invoice_status'=>'1',
						'payment_mode_desc'=>'Bank Card/Online',
						'confirm_invoice'=>date('Y-m-d H:i:s'),
						'payment_id'=>$insert_id,
	...
	public function list_pathtest_bytype()
	{
		...
		echo '<div class="form-group">
				<label>Charge Name</label>
					<select class="form-control input-sm select2" id="itype_name_id" name="itype_name_id"  >	';				
	...
	public function update_refer_doc()
	{
		...
		$inv_date=str_to_MysqlDate($this->input->post('inv_date'));
	...
	public function showinvoice($invoice_id)
	{
		...
		if($data['invoice_master'][0]->insurance_case_id>0)
		{
			$sql="select * from  organization_case_master 
			 where case_type=0 and id=".$data['invoice_master'][0]->insurance_case_id;
		}elseif(count($data['ipd_master'])>0){
			if($data['ipd_master'][0]->case_id>0)
			{
				$sql="select * from  organization_case_master  where case_type=0 and id=".$data['ipd_master'][0]->case_id;
			}
		}
	...
	public function invoice_PDF_print($invoice_id,$page_size=0)
	{
		...
		//   $this->m_pdf->pdf->showImageErrors = false;
